...
// 1. O seu useEffect original (Roda APENAS UMA VEZ na montagem da tela)
  ...
  useEffect(() => {
    const buscarHorariosOcupados = async () => {
      ...
      try {
        const res = await fetch(`http://localhost:5000/agendamentos/ocupados?data=${dataFormatada}&profissionalId=${profissionalSelecionado}`, {
  ...
  const carregarDadosIniciais = async () => {
    ...
    try {
      ...
      const resAgendamentos = await fetch("http://localhost:5000/agendamentos/meus", { headers });
